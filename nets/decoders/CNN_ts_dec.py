...
class CNN_TS_dec(torch.nn.Module):
    ...
    def __init__(self, inp_feats: int, hid_feats: int, out_feats: int, 
        ...
        self.cnn_0 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=inp_feats, out_channels=hid_feats*2, kernel_size=5, padding='same')),
        ...
        self.cnn_1 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=hid_feats*2, out_channels=hid_feats, kernel_size=3, padding='same')),
            ("bn", nn.BatchNorm1d(num_features=hid_feats)),
            ("activation", nn.ReLU())
            ]))
        
        self.last = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=hid_feats, out_channels=out_feats, kernel_size=1, padding="same")),
            ]))
