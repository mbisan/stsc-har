...
class UNET(nn.Module):
    ...
    def __init__(self, in_features, num_classes, latent_features) -> None:
        ...
        self.cnn_0 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=in_features, out_channels=latent_features, kernel_size=5, padding='same')),
        ...
        self.cnn_1 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=latent_features, out_channels=latent_features, kernel_size=3, padding='same')),
            ("bn", nn.BatchNorm1d(num_features=latent_features)),
            ("activation", nn.ReLU()), 
            ("pool", nn.MaxPool1d(kernel_size=2))
            ]))
        ...
        self.last = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=latent_features*2, out_channels=latent_features*4, kernel_size=1, padding="same")),
            ("activation", nn.ReLU())
            ]))
        
        self.tcnn_0 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=latent_features*4, out_channels=latent_features*2, kernel_size=3, padding='same')),
            ("bn", nn.BatchNorm1d(num_features=latent_features*2)),
            ("activation", nn.ReLU()), 
            ]))
        ...
        self.tcnn_1 = nn.Sequential(OrderedDict([
            ("conv", nn.Conv1d(in_channels=latent_features*2, out_channels=latent_features, kernel_size=3, padding='same')),
            ("bn", nn.BatchNorm1d(num_features=latent_features)),
            ("activation", nn.ReLU()), 
            ]))
